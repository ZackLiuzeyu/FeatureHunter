      ...
      for (dr in dropoutrate_grid) {
        ...
        for (k in seq_along(th_vec)) {
          ...
          if (ncol(mets) != (n_test + 2L)) {
            stop(sprintf(
              "Internal consistency error: expected %d columns, got %d. names: %s",
              n_test + 2L, ncol(mets),
          ...
          result_FS <- as.numeric(mets["F1", ])
          names(result_FS) <- names_now
          result_acc <- as.numeric(mets["ACC", ])
          names(result_acc) <- names_now
          result_recall <- as.numeric(mets["RECALL", ])
          names(result_recall) <- names_now
