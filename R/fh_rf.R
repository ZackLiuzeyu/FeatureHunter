...
fh_rf <- function(train_exp, train_labels, test_exp, labels_list, com_genes,
                  mnum = c(
                    round(stats::quantile(seq_along(com_genes), probs = 0.25)),
  ...
  for (mnumberi in mnum) {
    ...
    for (cuti in cutoff_grid) {
      ...
      result_recall <- sapply(all_result, function(x) {
        prd <- as.character(x$predict_result)
        ref <- as.character(x$real_label)
...
fh_lasso_rf <- function(train_exp, train_labels, test_exp, labels_list,
                        lassogene, fold,
                        mnum = c(
                          round(stats::quantile(seq_len(length(lassogene)), probs = 0.25)),
  ...
  for (mnumberi in mnum) {
    ...
    for (cuti in cutoff_grid) {
      ...
      key <- if (is_auto) {
        paste0(
          "RF+Lasso-CV:", fold, " fold (mtry=", mnumberi, ") (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", method_used, "))"
        )
      } else {
        paste0(
          "RF+Lasso-CV:", fold, " fold (mtry=", mnumberi, ") (cutoff:",
