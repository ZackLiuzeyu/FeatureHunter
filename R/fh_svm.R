  ...
  for (kerneli in kernel_all) {
    ...
    if (length(comd_val) >= 2) {
      ...
      if (!is.null(prob_val) && length(unique(prob_val[is.finite(prob_val)])) >= 2) {
        ...
        if (is.finite(th_auto) && th_auto > 0.01 && th_auto < 0.99) {
          ...
          message(sprintf(
            "[SVM %s] Auto threshold on B = %.4f (method=%s)",
    ...
    for (cuti in cutoff_now) {
      ...
      nice_names <- c(
        "DatasetA(Train)",
        paste0(
          "Dataset", LETTERS[1 + seq_along(test_exp)],
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-default (kernel: ", kerneli, ") (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", method_used, "))"
        )
      } else {
        paste0(
          "SVM-default (kernel: ", kerneli, ") (cutoff:",
...
fh_svm_best <- function(train_exp, train_labels, test_exp, labels_list,
  ...
  for (kerneli in kernel_all) {
    ...
    for (cuti in cutoff_now) {
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-default+Lasso-CV:", fold, " fold (kernel: ", kerneli, ") (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", auto_th_method, "))"
        )
      } else {
        paste0(
          "SVM-default+Lasso-CV:", fold, " fold (kernel: ", kerneli, ") (cutoff:",
          formatC(cuti, format = "f", digits = 4), ")"
        )
      }
  ...
  # ---- 1) svmLinear ----
  {
    ...
    model <- caret::train(labels ~ . - labels,
      data = train_expd,
    ...
    for (cuti in cutoff_now) {
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-CV:", fold, " fold (kernel: linear) (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", auto_th_method, "))"
        )
      } else {
        paste0(
          "SVM-CV:", fold, " fold (kernel: linear) (cutoff:",
          formatC(cuti, format = "f", digits = 4), ")"
        )
      }
  ...
  # ---- 2) svmRadial ----
  {
    ...
    for (cuti in cutoff_now) {
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-CV:", fold, " fold (kernel: radial) (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", auto_th_method, "))"
        )
      } else {
        paste0(
          "SVM-CV:", fold, " fold (kernel: radial) (cutoff:",
          formatC(cuti, format = "f", digits = 4), ")"
        )
      }
  ...
  # ---- 3) svmPoly ----
  {
    ...
    for (cuti in cutoff_now) {
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-CV:", fold, " fold (kernel: polynomial) (cutoff:auto(",
          formatC(th_auto, format = "f", digits = 4), ",", auto_th_method, "))"
        )
      } else {
        paste0(
          "SVM-CV:", fold, " fold (kernel: polynomial) (cutoff:",
          formatC(cuti, format = "f", digits = 4), ")"
        )
      }
  ...
  .run_caret_svm <- function(method_name, kernel_label) {
    ...
    if (length(comd_val) >= 2) {
      ...
      if (!is.null(prob_val) && length(unique(prob_val[is.finite(prob_val)])) >= 2) {
        ...
        if (is.finite(th_auto) && th_auto > 0.01 && th_auto < 0.99 && length(th_auto) == 1L && th_auto > 0.01 && th_auto < 0.99) {
          cutoff_now <- sort(unique(c(cutoff_now, th_auto)))
          message(sprintf(
            "[lasso+SVM+CV best %s] Auto threshold on B = %.4f (method=%s)",
    ...
    for (cuti in cutoff_now) {
      ...
      result_acc <- sapply(all_result, function(x) {
        mean(as.character(x$predict_result) ==
      ...
      key <- if (is_auto) {
        paste0(
          "SVM-CV:", fold, " fold + Lasso-CV:", fold,
