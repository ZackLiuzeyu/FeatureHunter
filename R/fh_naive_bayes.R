    ...
    if (!is.null(prob_val_df)) {
      ...
      if (!.is_degenerate_probs(prob_val)) {
        ...
        if (is.finite(th_auto) && th_auto > 0.01 && th_auto < 0.99) {
          ...
          message(sprintf(
            "[NaiveBayes] Auto threshold on B = %.4f (method=%s)",
            th_auto, method_used
          ))
        } else {
...
fh_naive_bayes_lasso <- function(train_exp,
    ...
    if (!is.null(prob_val_df)) {
      ...
      if (!.is_degenerate_probs(prob_val)) {
        ...
        if (is.finite(th_auto) && th_auto > 0.01 && th_auto < 0.99) {
          cutoff_now <- sort(unique(c(cutoff_now, th_auto)))
          message(sprintf(
            "[lasso+NaiveBayes] Auto threshold on B = %.4f (method=%s)",
            th_auto, method_used
          ))
        } else {
  ...
  for (cuti in cutoff_now) {
    ...
    key <- if (is_auto) {
      paste0(
        "lasso+NaiveBayes-CV:", fold, " fold (cutoff:auto(",
        formatC(th_auto, format = "f", digits = 4), ",", method_used, "))"
      )
    } else {
      paste0(
        "lasso+NaiveBayes-CV:", fold, " fold (cutoff:",
