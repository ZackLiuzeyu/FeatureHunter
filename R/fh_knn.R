  ...
  for (k_i in knumber) {
    ...
    for (cuti in cutoff_now) {
      ...
      all_result <- lapply(seq_along(test_exp), function(i) {
        ...
        if (!length(comd)) {
          return(data.frame(predict_p = numeric(0), predict_result = factor(character()), real_label = factor(character())))
        }
        expdata <- data_i[comd, , drop = FALSE]
        expdata[is.na(expdata)] <- colMeans(expdata, na.rm = TRUE)
        ...
        data.frame(
          predict_p = prob_i, predict_result = pr,
      ...
      key <- if (is_auto) {
        paste0("KNN (k=", k_i, ") (cutoff:auto(", formatC(th_auto, format = "f", digits = 4), ",", method_used, "))")
      } else {
        paste0("KNN (k=", k_i, ") (cutoff:", formatC(cuti, format = "f", digits = 4), ")")
      }
...
fh_lasso_knn <- function(train_exp, train_labels, test_exp, labels_list,
  ...
  fs_local <- list()
  summary_local <- list()
  train_imp <- as.data.frame(train_exp)[, lassogene, drop = FALSE]
  train_imp[is.na(train_imp)] <- colMeans(train_imp, na.rm = TRUE)
  y_fac <- factor(ifelse(train_labels == 1, "1", "0"), levels = c("0", "1"))
  ...
  for (k_i in knumber) {
    ...
    for (cuti in cutoff_now) {
      ...
      all_result <- lapply(seq_along(test_exp), function(i) {
        ...
        prob_i <- ifelse(as.character(pred_i) == "1", p_hat_i, 1 - p_hat_i)
        pr <- factor(ifelse(prob_i > cuti, "positive", "negative"))
        data.frame(predict_p = prob_i, predict_result = pr, real_label = factor(ifelse(as_binary01(labelsdata) == 1, "positive", "negative")))
      })
      all_result <- c(list(Train = train_result), all_result)
      names(all_result) <- c("DatasetA(Train)", paste0("Dataset", LETTERS[1:length(test_exp)], "(", c("Val", rep("Test", length(test_exp) - 1)), ")"))
